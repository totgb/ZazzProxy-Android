    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            node = new ZazzUdpNode(this, name, true, key, new ZazzUdpNode.Callback() {
                @Override public void onPeer(ZazzUdpNode.Peer peer) { update("Connected: " + peer.name); }
                @Override public void onManifest(ZazzUdpNode.Peer peer, List<ZazzUdpNode.ShareFile> files) { }
        ...
        } catch (Exception e) { update("Unable to start: " + e.getMessage()); stopSelf(); return START_NOT_STICKY; }
    }
    @Override public void onDestroy() { if (node != null) node.close(); super.onDestroy(); }
